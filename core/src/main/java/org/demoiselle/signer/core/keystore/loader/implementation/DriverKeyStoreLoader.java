...
public class DriverKeyStoreLoader implements KeyStoreLoader {
	...
	public KeyStore getKeyStoreFromDriver(String driverName, String driverPath) {
		...
		try {
			...
			AuthProvider aprov = (AuthProvider) pkcs11Provider.configure(pkcs11ConfigSettings);
			aprov.login(null, this.callback);
			Security.addProvider(aprov);

			keyStore = KeyStore.getInstance(PKCS11_KEYSTORE_TYPE, aprov.getName());
			keyStore.load(null, null);

		} catch (IOException | LoginException | IllegalArgumentException | SecurityException | KeyStoreException
				| NoSuchAlgorithmException | NoSuchProviderException | CertificateException ex) {
	...
	private KeyStore getKeyStoreFromConfigFile(String configFile) {
		...
		try {
			Provider pkcs11Provider = Security.getProvider("SunPKCS11");
			AuthProvider aprov = (AuthProvider) pkcs11Provider.configure(configFile);
			aprov.login(null, this.callback);
