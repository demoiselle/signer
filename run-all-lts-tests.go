...
func main() {
	...
	for _, version := range versions {
		fmt.Printf("\n------------------------------------------------------------\n")
		fmt.Printf("➜ Preparando ambiente e rodando testes para Java %s\n", version)
		fmt.Printf("------------------------------------------------------------\n")

		// Comando instala o Java se não existir, depois executa os testes
		cmdStr := fmt.Sprintf(`source "$HOME/.sdkman/bin/sdkman-init.sh" && sdk install java %s </dev/null && sdk use java %s && mvn clean test -pl core -B`, version, version)
