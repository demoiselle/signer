...
func main() {
	...
	for _, link := range p7bLinks {
		...
		crtPath := filepath.Join(crtFolder, strings.TrimSuffix(fileName, ".p7b")+".crt")
		var cmd *exec.Cmd
		if isPEM {
			// PKCS7 em PEM
			...
			// Se o OpenSSL funcionar, salva tudo em um único .crt
			err = os.WriteFile(crtPath, output, 0644)
			if err != nil {
			...
			// PKCS7 em DER
			cmd = exec.Command("openssl", "pkcs7", "-inform", "DER", "-print_certs", "-in", p7bPath, "-out", crtPath)
			output, err := cmd.CombinedOutput()
			if err != nil {
				msg := fmt.Sprintf("[ERRO] Converter: %s -> %s | %v\nSaída OpenSSL: %s\n", p7bPath, crtPath, err, string(output))
				fmt.Print(msg)
				logf.WriteString(msg)
			...
			msg = fmt.Sprintf("[OK] Convertido: %s -> %s\n", p7bPath, crtPath)
			fmt.Print(msg)
			logf.WriteString(msg)
		}
